...
const DIFFERENT_POT_FEE_CREATION: u64 = 2_000;
...
#[test]
fn test_change_fee_for_pots_twice() {
    let mut state = PotlockTestState::new();
    state.deploy_potlock_contract();
    state.change_fee_for_pots(POT_FEE_CREATION);

    state.change_fee_for_pots(POT_FEE_CREATION * 2);
...
#[test]
fn test_fail_add_pot_wrong_payment() {
    let mut state = PotlockTestState::new();
    state.deploy_potlock_contract();
    state.change_fee_for_pots(POT_FEE_CREATION);
    ...
    state
        ...
        .with_result(ExpectError(4, "Wrong token identifier for creating a pot!"))
        .run();
    ...
    state
        ...
        .egld_or_single_esdt(
                &EgldOrEsdtTokenIdentifier::esdt(POT_TOKEN_ID),
                0u64,
                &multiversx_sc::proxy_imports::BigUint::from(DIFFERENT_POT_FEE_CREATION),
        )
        .with_result(ExpectError(4, "Wrong fee amount for creating a pot"))
        .run();
...
#[test]
fn test_fail_change_fee_for_pots()
{
    let mut state = PotlockTestState::new();
...
#[test]
fn test_fail_accept_pot_non_existent() {
    let mut state = PotlockTestState::new();
    state.deploy_potlock_contract();
    state.change_fee_for_pots(POT_FEE_CREATION);

    let potlock_id = 0usize;
    state
        .world
        .tx()
        .from(OWNER_ADDRESS)
        ...
        .typed(potlock_proxy::PotlockProxy)
        .accept_pot(potlock_id)
        .with_result(ExpectError(4, "Potlock doesn't exist!"))
        .run();
}
...
#[test]
fn test_fail_remove_pot_non_existent() {
    let mut state = PotlockTestState::new();
    state.deploy_potlock_contract();
    state.change_fee_for_pots(POT_FEE_CREATION);
...
#[test]
fn test_fail_donate_to_inactive_project() {
    let mut state = PotlockTestState::new();
    state.deploy_potlock_contract();
    state.change_fee_for_pots(POT_FEE_CREATION);
...
#[test]
fn test_fail_donate_to_non_existing_pot(){
    let mut state = PotlockTestState::new();
    state.deploy_potlock_contract();
    state.change_fee_for_pots(POT_FEE_CREATION);
